...
class KickChatClient:
    ...
    def _parse_chat_message(self, message: dict) -> Optional[dict]:
        ...
        return {
            ...
            "user": username,
            "message": content,
            "badges": self._extract_badges(sender if isinstance(sender, dict) else {}),
        }

    def _extract_badges(self, sender: dict) -> list[dict[str, str]]:
        if not sender:
            return []

        badges: list[dict[str, str]] = []
        raw_badges = sender.get("badges")
        if isinstance(raw_badges, list):
            for entry in raw_badges:
                if not isinstance(entry, dict):
                    continue
                label = (
                    entry.get("badge_name")
                    or entry.get("name")
                    or entry.get("slug")
                    or entry.get("type")
                )
                badge_id = (
                    entry.get("badge_id")
                    or entry.get("id")
                    or (label.lower().replace(" ", "_") if label else None)
                )
                badge_payload: dict[str, str] = {}
                if badge_id:
                    badge_payload["id"] = str(badge_id)
                if label:
                    badge_payload["label"] = str(label)
                icon = entry.get("badge_image") or entry.get("image") or entry.get("icon")
                if icon:
                    badge_payload["icon"] = icon
                if badge_payload:
        ...
        def flag_true(key: str) -> bool:
            ...
            return bool(sender.get(camel_key))

        fallback_flags = [
            ("is_broadcaster", "broadcaster", "Broadcaster"),
            ("is_moderator", "moderator", "Mod"),
            ("is_vip", "vip", "VIP"),
            ("is_subscriber", "subscriber", "Sub"),
        ]

        existing_ids = {badge.get("id") for badge in badges if badge.get("id")}
        for key, badge_id, label in fallback_flags:
            if badge_id in existing_ids:
                continue
            if flag_true(key):
                badges.append({"id": badge_id, "label": label})
